                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            auto const& parentNode = std::find_if(_nodes.begin(), _nodes.end(), [&parentToken](std::unique_ptr<GraphNode> &node) { return node.get()->GetID() == std::stoi(parentToken->second); });
                            auto const& childNode = std::find_if(_nodes.begin(), _nodes.end(), [&childToken](std::unique_ptr<GraphNode> &node) { return node.get()->GetID() == std::stoi(childToken->second); });
                            ...
                            (*childNode)->AddEdgeToParentNode(edge.get());       //edited from feedback
                            (*parentNode)->AddEdgeToChildNode(std::move(edge));  //edited from feedback
                        }
